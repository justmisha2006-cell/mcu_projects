...
const char* firmwareURL =
  "https://raw.githubusercontent.com/ua2026-hash/TargetiX_OS/main/firmware_sl7.bin";
...
<head>
  ...
  <style>
    :root {
      ...
      --danger: #ef4444;
      --danger-hover: #dc2626;
      --badge-full-bg: #dcfce7;
      --badge-full-text: #15803d;
      --badge-lock-bg: #e2e8f0;
      --badge-lock-text: #64748b;
      --cubic-smooth: cubic-bezier(0.16, 1, 0.3, 1);
    }
    ...
    * {
    ...
    .card:nth-child(1) { animation: fadeUp 0.6s var(--cubic-smooth) 0.15s forwards; }
    .card:nth-child(2) { animation: fadeUp 0.6s var(--cubic-smooth) 0.25s forwards; }
    .card:nth-child(3) { animation: fadeUp 0.6s var(--cubic-smooth) 0.35s forwards; }
    .card:nth-child(4) { animation: fadeUp 0.6s var(--cubic-smooth) 0.45s forwards; }
    .card:nth-child(n+5) { animation: fadeUp 0.6s var(--cubic-smooth) 0.55s forwards; }

    .card:hover:not(.locked-card):not(.navigating) {
      transform: translateY(-2px);
      box-shadow: 0 12px 22px -4px rgba(15, 23, 42, 0.08);
      border-color: var(--text-sub);
    }

    .card:active:not(.locked-card) {
      transform: translateY(1px) scale(0.98);
      box-shadow: 0 2px 6px -1px rgba(15, 23, 42, 0.04);
    }

    /* Защита от двойного клика (состояние перехода) */
    .card.navigating {
      pointer-events: none;
      opacity: 0.6;
    ...
    .card:hover:not(.locked-card):not(.navigating) .action-icon {
      background: var(--primary);
      border-color: var(--primary);
      color: #ffffff;
      transform: translateX(4px);
    ...
    .badge.lock {
      background: var(--badge-lock-bg);
      color: var(--badge-lock-text);
    }
    ...
    .stop-btn {
      ...
      cursor: pointer;
      display: none;
      box-shadow: 0 10px 25px -4px rgba(239, 68, 68, 0.45);
      transition: transform 0.3s var(--cubic-smooth);
      animation: stopPop 0.4s var(--cubic-smooth) forwards;
    }
    ...
    /* Блок системной информации (для ошибок связи) */
    .system-info {
      margin: 24px auto 0;
      padding: 0 16px;
      text-align: center;
  ...
  <script>
    ...
    function goUrl(url, element) {
      if (isNavigating) return; 
      isNavigating = true;
      triggerFeedback();

      if (element) element.classList.add('navigating');
    ...
    function runProgram(url, id) {
      if (programRunning || isNavigating) return;
      triggerFeedback();
      programRunning = true; 
      currentId = id; 
      lockButtons(true);
    ...
    function lockButtons(state) {
      document.querySelectorAll('.program-btn').forEach(b => b.classList.toggle('locked-card', state));
    }

    // Обработка потери связи (блок статуса отображается только при потере связи)
    function updateSystemStatus(isOnline, langTextOffline) {
      const sysWrapper = document.getElementById('sysInfoWrapper');
      const sysBox = document.getElementById('sysStatusBox');
      
      if (!isOnline) {
        sysWrapper.classList.add('visible');
        sysBox.classList.add('offline');
        sysBox.innerHTML = '⚠️ ' + langTextOffline;
        lockButtons(true);
      } else {
        sysWrapper.classList.remove('visible');
      }
    }
    ...
    async function upd() {
      ...
      try {
        ...
        let r = await fetch('/STATUS', { signal: controller.signal });
        ...
        if (t.startsWith('RUNNING')) {
          ...
          lockButtons(true);
          let id = t.split(':')[1];
          let btn = document.querySelector('.program-btn[data-id="' + id + '"]');
        ...
        const sysBox = document.getElementById('sysStatusBox');
        updateSystemStatus(true, sysBox.dataset.offline);

      } catch(e) {
        failCount++;
        if (failCount >= 3) {
          const sysBox = document.getElementById('sysStatusBox');
          updateSystemStatus(false, sysBox.dataset.offline);
        }
      }
    }
    ...
    function lpEnd() { clearTimeout(pressTimer); }

    function delCard() { 
      triggerFeedback();
      fetch('/DELCARD?id=' + delCardId).then(() => location.reload()).catch(()=>{}); 
    }
...
<body>
  ...
  <div class="container">
    <header>
      <div class="header-title">TargetiX</div>
    </header>

    <div class="grid">
)raw";

  /* Настройки. Формат tr: (Немецкий, Английский, Украинский) */
  html += "<div class='card' onclick=\"goUrl('/SETTINGS', this)\">";
  html += "<div class='card-content'>";
  html += "<div class='title'>⚙️ " + tr("Einstellungen", "Налаштування", "Settings") + "</div>";
  html += "<div class='sub'>" + tr("Systemparameter", "Параметри системи", "System config") + "</div>";
  html += "</div>";
  html += "<div class='action-icon'><svg width='20' height='20' viewBox='0 0 24 24' fill='none' stroke='currentColor' stroke-width='2.5' stroke-linecap='round' stroke-linejoin='round'><path d='M9 18l6-6-6-6'/></svg></div>";
  html += "</div>";

  /* Пресет MP5 */
  html += "<div class='card program-btn' data-id='MP5' onclick=\"runProgram('/RUNPRESET?mode=MP5','MP5')\">";
  html += "<div class='card-content'>";
  html += "<div class='title'>🎯 " + modeName("MP5") + "</div>";
  html += "<div class='sub'>" + tr("Schnellstart", "Швидкий запуск", "Quick start") + "</div>";
  html += "</div>";
  html += "<div class='action-icon'><svg width='20' height='20' viewBox='0 0 24 24' fill='none' stroke='currentColor' stroke-width='2.5' stroke-linecap='round' stroke-linejoin='round'><polygon points='5 3 19 12 5 21 5 3'/></svg></div>";
  html += "</div>";

  /* Пресет MP8 */
  html += "<div class='card' onclick=\"goUrl('/MP8', this)\">";
  html += "<div class='card-content'>";
  html += "<div class='title'>🎯 " + modeName("MP8") + "</div>";
  html += "<div class='sub'>" + tr("Profilauswahl", "Вибір профілю", "Select profile") + "</div>";
  html += "</div>";
  html += "<div class='action-icon'><svg width='20' height='20' viewBox='0 0 24 24' fill='none' stroke='currentColor' stroke-width='2.5' stroke-linecap='round' stroke-linejoin='round'><path d='M9 18l6-6-6-6'/></svg></div>";
  html += "</div>";

  /* Пресет MP10 */
  html += "<div class='card' onclick=\"goUrl('/MP10', this)\">";
  html += "<div class='card-content'>";
  ...
  for (uint8_t i = 0; i < cardCount; i++) {
    html += "<div class='card' onmousedown='lpStart(" + String(i) + ")' onmouseup='lpEnd()' ontouchstart='lpStart(" + String(i) + ")' ontouchend='lpEnd()' onclick=\"goUrl('/CARD?id=" + String(i) + "', this)\">";
    html += "<div class='card-content'>";
    html += "<div class='title'>📁 " + String(cards[i].title) + "</div>";
    html += "<div class='sub'>" + tr("Programme: ", "Програми: ", "Programs: ") + String(cards[i].count) + "</div>";
    html += "</div>";
    html += "<div class='action-icon'><svg width='20' height='20' viewBox='0 0 24 24' fill='none' stroke='currentColor' stroke-width='2.5' stroke-linecap='round' stroke-linejoin='round'><path d='M9 18l6-6-6-6'/></svg></div>";
    html += "</div>";
  }

  /* Добавление новых карточек (Разблокировано) */
  html += "<div class='card' onclick=\"goUrl('/ADD', this)\">";
  html += "<div class='card-content'>";
  html += "<div class='title'>➕ " + tr("Hinzufügen", "Додати", "Add") + "</div>";
  html += "<div class='sub'>" + tr("Neues Programm", "Нова програма", "New program") + "</div>";
  html += "</div>";
  html += "<div class='action-icon'><svg width='20' height='20' viewBox='0 0 24 24' fill='none' stroke='currentColor' stroke-width='2.5' stroke-linecap='round' stroke-linejoin='round'><path d='M9 18l6-6-6-6'/></svg></div>";
  html += "</div>";
  html += "</div>";  // Закрытие блока grid

  html += R"raw(
    </div>

    <div id="stopBtn" class="stop-btn" onclick="stopOnly()">⛔ )raw"
          + tr("STOPP", "СТОП", "STOP") + R"raw(</div>

    <!-- Блок статуса связи (скрыт в штатном режиме) -->
    <div id="sysInfoWrapper" class="system-info">
      <div id="sysStatusBox" class="system-info-box" 
           data-offline=")raw"
          + tr("Verbindung unterbrochen", "Зв'язок втрачено", "Connection Lost") + R"raw(">
      </div>
    </div>
  </div>
  ...
  <div id="overlay" class="overlay">
    <div class="modal">
      <h2>)raw"
          + tr("Karte löschen?", "Видалити картку?", "Delete card?") + R"raw(</h2>
      <button class="btn danger" onclick="delCard()">)raw"
...
<head>
  ...
  <style>
    ...
    * {
    ...
    /* Каскадное появление */
    .card:nth-child(1) { animation: fadeUp 0.6s var(--cubic-smooth) 0.2s forwards; }
    .card:nth-child(2) { animation: fadeUp 0.6s var(--cubic-smooth) 0.3s forwards; }
    .card:nth-child(3) { animation: fadeUp 0.6s var(--cubic-smooth) 0.4s forwards; }
    ...
    .stop-btn:hover {
      ...
      transform: translateY(-2px);
      box-shadow: 0 14px 28px -4px rgba(239, 68, 68, 0.55);
    }
    ...
    /* Универсальная анимация подъема */
    @keyframes fadeUp {
      from {
        opacity: 0;
        transform: translateY(14px);
  ...
  <script>
    ...
    function lockCards(state) {
      document.querySelectorAll('.card').forEach(c => {
        c.classList.toggle('locked', state);
      });
    }
  ...
  <div class="container">
    ...
    <div class="grid">
      <div class="card" onclick="runMP8('/RUNPRESET?mode=MP8&g=8')">
        <div class="card-content">
          <div class="title">⏱️ )====="
         + tr("8 Sekunden", "8 секунд", "8 seconds") + R"=====(</div>
          <div class="sub">)====="
         + tr("Schussserie", "Серія пострілів", "Shot series") + R"=====(</div>
        </div>
        <div class="action-icon">
      ...
      <div class="card" onclick="runMP8('/RUNPRESET?mode=MP8&g=6')">
        <div class="card-content">
          <div class="title">⏱️ )====="
         + tr("6 Sekunden", "6 секунд", "6 seconds") + R"=====(</div>
          <div class="sub">)====="
         + tr("Schussserie", "Серія пострілів", "Shot series") + R"=====(</div>
        </div>
        <div class="action-icon">
      ...
      <div class="card" onclick="runMP8('/RUNPRESET?mode=MP8&g=4')">
        <div class="card-content">
          <div class="title">⏱️ )====="
         + tr("4 Sekunden", "4 секунди", "4 seconds") + R"=====(</div>
          <div class="sub">)====="
         + tr("Schussserie", "Серія пострілів", "Shot series") + R"=====(</div>
        </div>
        <div class="action-icon">
    ...
    <div id="stopBtn" class="stop-btn" onclick="stopOnly()">⛔ )====="
         + tr("STOPP", "СТОП", "STOP") + R"=====(</div>
  </div>
  ...
  <div class="container">
    ...
    <div class="grid">
      <div class="card" onclick="runMP10('/RUNPRESET?mode=MP10&g=150')">
        <div class="card-content">
          <div class="title">⏱️ )====="
         + tr("150 Sekunden", "150 секунд", "150 seconds") + R"=====(</div>
          <div class="sub">)====="
         + tr("Schussserie", "Серія пострілів", "Shot series") + R"=====(</div>
        </div>
        <div class="action-icon">
      ...
      <div class="card" onclick="runMP10('/RUNPRESET?mode=MP10&g=20')">
        <div class="card-content">
          <div class="title">⏱️ )====="
         + tr("20 Sekunden", "20 секунд", "20 seconds") + R"=====(</div>
          <div class="sub">)====="
         + tr("Schussserie", "Серія пострілів", "Shot series") + R"=====(</div>
        </div>
        <div class="action-icon">
      ...
      <div class="card" onclick="runMP10('/RUNPRESET?mode=MP10&g=10')">
        <div class="card-content">
          <div class="title">⏱️ )====="
         + tr("10 Sekunden", "10 секунд", "10 seconds") + R"=====(</div>
          <div class="sub">)====="
         + tr("Schussserie", "Серія пострілів", "Shot series") + R"=====(</div>
        </div>
        <div class="action-icon">
...
if (request.indexOf("/api/ready") >= 0) {
  ...
  if (
    ...
    client.println("Location: http://192.168.2.7/");
    client.println("Connection: close");
    client.println();
